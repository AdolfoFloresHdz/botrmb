...
class MessageHandler {
  ...
  async handleIncomingMessage(message, senderInfo) {
    ...
    if (message?.type === "text") {
      ...
      if (this.isGreeting(incomingMessage)) {
        ...
        const state = this.appointmentState[fromNumber];
        if (state?.step) {
  ...
  async sendWelcomeMessage(to, messageId, senderInfo) {
    ...
    const welcomeMessage = `Hola, ${name}, bienvenido a nuestro servicio de RMB. ¿En qué puedo ayudarte hoy?`;
  ...
  async handleMenuOption(to, option) { 
    ...
    switch(option) {
      ...
      case 'option_3':
        ...
        break;
      case 'option_6':
        response = 'si esto es una emergencias te invitamos a llamar a nuestra linea de atencion'
        await this.sendContact(to)
        break;
      default:
        response = 'Lo siento, no entiendo tu selección. Elige una opción válida.';
    }
  ...
  async handleAppointmentFlow(to, message) {
    ...
    switch(state.step) {
      case 'name':
        state.name = message;
        state.step = 'petName';
        response = 'Gracias, ahora dime, ¿Cuál es el nombre de la mascota?';
        break;  
      case 'petName':
        state.petName = message;
        state.step = 'petType';
        response = '¿Cuál es el tipo de mascota? (por ejemplo: perro, gato, hurón, etc.)';
        break;
      case 'petType':
...
async sendLocation(to) {
  ...
  const addresses = 'Cra. 43A #5A-113, El Poblado, Medellín, Antioquia.';
